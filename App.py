...
if primer_type == "RACE":
    with st.expander("⚙️ Change design parameters", expanded=False):
        ...
        race_params["window_size"] = col1.number_input(
            "🔍 Search Window Size (bp)", 
            value=200, 
            min_value=50, 
            on_change=clear_output,
            help="Size of the search window from the 5' and 3' ends of the sequence"
        )
        race_params["k_range"] = col1.slider(
            "📏 Primer Length (k-mer)", 
            value=(23, 29), 
            min_value=15, 
            max_value=30, 
        ...
        race_params["gc_range"] = col2.slider(
            "🧬 GC Content (%)", 
            value=(0.5, 0.7), 
            min_value=0.0, 
            max_value=1.0, 
...
if primer_type == "qPCR":
    with st.expander("⚙️ Change design parameters", expanded=False):
        ...
        # ❌ ELIMINADO: fw_range y rev_range
        # Para qPCR, los primers se buscan en TODA la secuencia
        
        qpcr_params["k_range"] = col1.slider(
            "📏 Primer Length (k-mer)", 
            value=(18, 26), 
            min_value=15, 
            max_value=30, 
            on_change=clear_output,
            help="Range of acceptable primer lengths"
        )
        qpcr_params["gc_range"] = col1.slider(
            "🧬 GC Content (%)", 
            value=(0.5, 0.7), 
            min_value=0.0, 
            max_value=1.0, 
            on_change=clear_output,
            help="Acceptable range of GC content"
        )
        qpcr_params["prod_range"] = col2.slider(
            "💥 Product Size (bp)", 
            value=(80, 150), 
            min_value=50, 
            max_value=300, 
            on_change=clear_output,
            help="Desired PCR amplicon size range"
        )
        qpcr_params["tm_range"] = col2.slider(
            ...
            max_value=100.0, 
            on_change=clear_output,
            help="Target melting temperature range for primers"
        )
...
if st.session_state.output_df1 is not None:
    ...
    st.dataframe(st.session_state.output_df1, use_container_width=True)
...
if st.session_state.output_df2 is not None:
    ...
    st.dataframe(st.session_state.output_df2, use_container_width=True)
